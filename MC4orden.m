function [sol,incre,increfx,iter,ACOC] = MC4orden(f,x0,tol,maxiter)
% Método de orden 4 con media cuadrática para aproximar una solución de f(x)=0 
%Ejemplo de ejecución en consola:
%[sol,incre,increfx,iter,ACOC,x] = MC4orden('f',0.5,1e-40,500)
...
while incre+increfx > tol && iter < maxiter
    ...
    [~,dfuny]=feval(f,y);
    t=dfuny/dfun;
    x1=x0-(7/8*t^2-5/2*t+21/8)*fun/sqrt(1/2*(dfun^2+dfuny^2));
    incre = norm(x1-x0);
