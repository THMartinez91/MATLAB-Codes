function [sol,incre,increfx,iter,ACOC,x] = MH4orden(f,x0,tol,maxiter)
% Método de orden 4 con media Heroniana para aproximar una solución de f(x)=0 
%Ejemplo de ejecución en consola:
%[sol,incre,increfx,iter,ACOC,x] = MH4orden('f',0.5,1e-40,500)
...
while incre+increfx > tol && iter < maxiter
    ...
    [~,dfuny]=feval(f,y);
    t=dfuny/dfun;
    x1=x0-(17/24*t^2-5/3*t+47/24)*3*fun/(dfun+dfuny+sqrt(dfun*dfuny));
    incre = norm(x1-x0);
